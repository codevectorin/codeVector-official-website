  ...
  function d(t, n) {
    ...
    setTimeout(e, 1e3 / 60);
      ...
      var t = this,
          ...
          i = this._getScrollTop();if (this.particles !== r) for (var s = this.particles.length - 1; s >= 0; s--) this.particles[s].$element.data("stellar-elementIsActive", r);this.particles = [];if (!this.options.parallaxElements) return;this.$element.find("[data-stellar-ratio]").each(function (n) {
        var i = e(this),
            s,
            o,
            u,
      ...
      var e, t, n, r, i;for (i = this.particles.length - 1; i >= 0; i--) e = this.particles[i], t = e.$element.data("stellar-startingLeft"), n = e.$element.data("stellar-startingTop"), this._setPosition(e.$element, t, t, n, n), this.options.showElement(e.$element), e.$element.data("stellar-startingLeft", null).data("stellar-elementIsActive", null).data("stellar-backgroundIsActive", null);for (i = this.backgrounds.length - 1; i >= 0; i--) r = this.backgrounds[i], r.$element.data("stellar-backgroundStartingLeft", null).data("stellar-backgroundStartingTop", null), c(r.$element, r.startingValueLeft, r.startingValueTop);
    }, destroy: function () {
...
(function (d, f, g, b) {
  if (!d) {
    console.error("jQuery nÃ£o encontrado, seu plugin jQuery nÃ£o irÃ¡ funcionar.");return false;
  }(function () {
    var k = 0,
        l = ["ms", "moz", "webkit", "o"];for (var j = 0; j < l.length && !f.requestAnimationFrame; ++j) {
      f.requestAnimationFrame = f[l[j] + "RequestAnimationFrame"];f.cancelAnimationFrame = f[l[j] + "CancelAnimationFrame"] || f[l[j] + "CancelRequestAnimationFrame"];
    }if (!f.requestAnimationFrame) {
      f.requestAnimationFrame = function (q, n) {
        var m = new Date().getTime();var o = Math.max(0, 16 - (m - k));var p = f.setTimeout(function () {
          q(m + o);
        }, o);k = m + o;return p;
      };
      ...
      a = d(f),
      i = 0;function h(k, j) {
    this._name = e;this._instance_id = ++i;this.el = k;this.$el = d(k);this.settings = d.extend(false, {}, c, j, this.$el.data());this.$triggerOrigin = d(this.settings.listenTo);this.init();
  }d.extend(h.prototype, { init: function () {
      var j = this;this.$triggerOrigin.on(j.settings.on + "." + j._name, function () {
        j.parallaxTranslate();
      });j.parallaxTranslate();
    }, parallaxTranslate: function () {
      var j = this;if (j.inScreen()) {
        f.requestAnimationFrame(function () {
          var k = a.scrollTop() - j.$el.offset().top;j.$el.css("transform", "translateY(" + k / 2 + "px)");
        });
      }console.groupEnd();
    }, destroy: function () {
      this.$el.removeData();d(this.settings.listenTo).off("." + e);
    }, somePublicMethod: function (k, j) {
      privateMethod.call(this);
    }, inScreen: function (l) {
      var o;if (typeof l !== "boolean" && l !== b) {
        o = d(l);l = arguments[1] || false;
      } else {
        o = this.$el;l = l || false;
      }var k = a.scrollTop(),
          n = k + a.height(),
          j = o.offset().top,
          m = j + o.height();if (l === true) {
        return k <= j && n >= m;
      }return !(k > m || n < j);
    } });d.fn[e] = function (k) {
    var j = arguments;if (k === b || typeof k === "object") {
      ...
      if (typeof k === "string" && k !== "init") {
        return this.each(function () {
          var l = d.data(this, "plugin_" + e);if (l instanceof h && typeof l[k] === "function") {
